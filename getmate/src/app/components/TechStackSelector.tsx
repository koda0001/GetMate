...
export function TechStackSelector({ initial = [], name = "techStack", mode = "edit" }: { initial?: string[], name?: string, mode?: 'edit' | 'view'}) {
      ...
      {isEdit ? (
        <div>
          <div className="flex flex-wrap gap-2 mb-2">
              {tags.map(tag => (
                ...
                <button
                  type="button"
                  className="ml-1 px-1 text-[#30364F] hover:text-red-600 font-bold"
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {tags.map(tag => (
            <span
            key={tag}
            className="inline-flex items-center px-3 py-1 bg-[#E1D9BC] border-2 border-[#30364F] rounded-sm font-mono text-xs font-bold shadow-[2px_2px_0_#30364F]"
            >
            {tag}
            <input type="hidden" name={name} value={tag} />
          </span>
